...
if __name__ == "__main__":
    ...
    # Als IN_DOCKER=1, dan draaien we zonder SSL en zonder threads (voor Docker)
    in_docker = os.environ.get("IN_DOCKER") == "1"

    if in_docker:
        # In Docker: HTTP, single-threaded
        microweb_app.run(host="0.0.0.0", port=5555, threaded=False)
    else:
        # In de VM (niet Docker): HTTPS zoals je test-script verwacht
